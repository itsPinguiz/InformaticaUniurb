...
loopM:
  ...
  L.D f6, 0(r2)      ; leggi valore cella
...
skip0:
  S.D f3, -8(r2)      ; salvataggio risultato
  
  C.LE.D f6, f2      ; f6 <= 0.3
  BC1F skip1        ; se falso salta
  MOV.D f6, f0       ; la cella corrente muore
skip1:
  S.D f6, 0(r2)      ; salvataggio risultato
  DADDI r2, r2, 8    ; passo all'elemento successivo
...
loopE:
  ...
  ; la posizione di sinistra dell'elemento successivo è la posizione dell'elemento corrente quindi DADDI r12, r10, 0, ma si può omettere perché r10 è la posizione corrente.
  DADDI r13, r10, 2   ; posizione vicino di destra -> w

  ; set if less than immediate (online è sbagliato)
  SLTI r11, r10, 1    ; r11 = 1 se r10 < 1 altrimenti r11 = 0

  SLT r14, r13, r9    ; r14 = 1 se r13 < 16 altrimenti r14 = 0
  
  BNEZ r6, skip2       ; se non è uguale a zero fa il salto.
...
skip2: 
  BEQZ r15, skip3       ; se è uguale a zero fa il salto.
...
skip5:
  ...
  BNEZ r11, skip6       ; se non è uguale a zero fa il salto.
    DADDI r7, r2, -8    ; puntatore di sinistra
    L.D f4, 0(r7)       ; leggi valore del vicino di sinistra -> grid[j]
    C.EQ.D f4, f0       ; grid[j] == 0
