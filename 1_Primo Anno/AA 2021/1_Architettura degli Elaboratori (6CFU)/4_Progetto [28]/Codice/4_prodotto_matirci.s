...
loop:   L.D f1, 0(r2)           ; leggi a[i]
        ...
        MUL.D   f5, f5, f6      ; a[i]*b[i]
        MUL.D   f7, f7, f8      ; a[i]*b[i]
    	ADD.D   f9, f3, f1      ; p = p + a[i]*b[i] (risultato parziale)
    	ADD.D   f10, f5, f7     ; p = p + a[i]*b[i] (risultato parziale)
    	ADD.D   f0, f10, f9     ; p = p + a[i]*b[i] (risultato parziale)
...
pointers:	DADDI   r4, r4, 8	; scorri registro risultati
		DADDI   r3, r3, 8	    ; punta alla nuova colonna
